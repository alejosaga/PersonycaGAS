...
function buscarEnOtroSheet() {
  ...
  if (!emailSheet || !aprobacionCotSheet) {
      Logger.log('No se encontró una de las hojas especificadas.');
      return;
    }
  
    // Obtener todos los datos del sheet de emails y del sheet de aprobaciones
    ...
    cotizaciones.forEach(cotizacion => {
      ...
      // Buscar el valor de la cotización en la hoja de aprobaciones
      aprobacionCotData.forEach(row => {
      ...
      if (resultado) {
        // Extraer la URL del archivo PDF de la penúltima columna y el enlace del formulario prellenado de la última columna
        let fileUrl = resultado[resultado.length - 2]; // Penúltima columna
        let formLink = resultado[resultado.length - 1]; // Última columna
        ...
        emailData.forEach(emailRow => {
          if (emailRow.includes(cotizacionValue)) {
            ...
            nombreContacto = emailRow[6]; // Asumiendo que el nombre de contacto está en la columna G (índice 6)
            servicioInteres = emailRow[14]; // Asumiendo que el servicio de interés está en la columna O (índice 14)
          }
        });
